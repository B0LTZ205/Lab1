...
<div class="container mt-3">
  <a href="LabCreate.php" class="btn btn-success float-end mb-3">Add Customer</a>
  ...
  while ($rows = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    echo "<td>{$Status}</td>";
    echo "<td>";
        echo "<a href='LabEdit.php?id={$BorrowID}' class='btne'>Edit</a>";
        echo "<a href='#' onclick='delete_user({$BorrowID});' class='btn1 ms-3'>Delete</a>";
      echo "</td>";
    echo "</tr>"; 
  ...
  <script>
    function delete_user(BorrowID){
      var answer = confirm("Are you sure you want to delete" + BorrowID + "?");
      if (answer) {
        window.location.href = 'LabDelete.php?id='+BorrowID;
        } else {
        // Do nothing!
      }
    }
